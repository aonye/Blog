...
export const users_index_get = async (req, res) => {
    ...
    console.log(users);
    return users && users.length !== 0 ? res.status(200).json(users) : res.status(400).json({ error: 'No users' });
};
...
export const user_post = [
    ...
    async (req, res) => {
        ...
        return result ? res.status(200).json({ msg: 'User successfully added' }) : res.status(400).json({ error: 'Error saving user' });
...
export const user_put = [
    body('username').trim()
        ...
        .isEmail().withMessage('Entry is not an email.')
        .custom((username, { req }) => {
            return User.findById(req.params.userId).then((user) => { // check if the name is unchanged
